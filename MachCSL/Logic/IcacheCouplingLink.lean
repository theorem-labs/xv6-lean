...
/-- Source coupling cameras occupy separate slots 28/29/30. -/
def registry : BundledGFunctors :=
  ((FsInodeRegion.registry.set 28 countFunctor).set 29 mirrorFunctor).set 30 pinFunctor

theorem registry_old (i : Nat) (bound : i < 28) : registry i = FsInodeRegion.registry i := by
  simp [registry, BundledGFunctors.set, show i ≠ 28 by omega, show i ≠ 29 by omega, show i ≠ 30 by omega]
theorem registry_unused (i : Nat) (bound : 31 ≤ i) : registry i = FsInodeRegion.registry i := by
  simp [registry, BundledGFunctors.set, show i ≠ 28 by omega, show i ≠ 29 by omega, show i ≠ 30 by omega]
@[reducible] def countSlot : ElemG registry CountRF := ⟨28, rfl⟩
@[reducible] def mirrorSlot : ElemG registry MirrorRF := ⟨29, rfl⟩
@[reducible] def pinSlot : ElemG registry PinRF := ⟨30, rfl⟩
def registryCapacity : Capacity registry := ⟨countSlot, mirrorSlot, pinSlot⟩
...
theorem count_slot : registryCapacity.count.τ = 28 := rfl
theorem mirror_slot : registryCapacity.mirror.τ = 29 := rfl
theorem pin_slot : registryCapacity.pin.τ = 30 := rfl
