...
def registry : BundledGFunctors := IcacheEscrowTokens.registry.set 38 armFunctor

theorem registry_old (i : Nat) (bound : i < 38) : registry i = IcacheEscrowTokens.registry i := by
  simp [registry, BundledGFunctors.set, show i ≠ 38 by omega]
theorem registry_unused (i : Nat) (bound : 39 ≤ i) : registry i = IcacheEscrowTokens.registry i := by
  simp [registry, BundledGFunctors.set, show i ≠ 38 by omega]
...
def nativeCapacity : Capacity registry := ⟨⟨⟨38, rfl⟩⟩, topCapacity, transactionCapacity⟩
theorem arm_slot : nativeCapacity.arms.elem.τ = 38 := rfl
theorem native_top_same : nativeCapacity.top = topCapacity := rfl
theorem native_transactions_same : nativeCapacity.transactions = transactionCapacity := rfl
