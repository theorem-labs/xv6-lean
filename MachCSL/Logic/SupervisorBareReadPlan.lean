...
open Iris MachCSL.Machine LeanPaperStock.Functions SupervisorRead
...
private theorem widen {small large : RegisterFootprint.Footprint} {rs : RegisterFile}
    ...
    (cut : OneRead small rs address program value)
    (members : ∀ cell, cell ∈ small → cell ∈ large) : OneRead large rs address program value := by
...
private theorem prefix_except {fp rs address} {segment : SailME ε β} {value : β}
    ...
    OneRead fp rs address (segment >>= next).run result := OneRead.prefix before after

private theorem one_lift {fp rs address} {program : SailM α} {value}
    (before : OneRead fp rs address program value) (ε : Type) :
    OneRead fp rs address (monadLift program : SailME ε α).run (fun word => Except.ok (value word)) := by
...
theorem outer_boundary (shares : Shares) (rs : RegisterFile) (address : BitVec 64)
    ...
    OneRead (footprint shares) rs address
      (mem_read (.Load .Data) .PBMT_PMA (.Physaddr address) 8 false false false)
      (fun word => .Ok word) := by
  change OneRead _ _ _ (SupervisorMemOuter.readProgram .data address) _
  rw [SupervisorMemOuter.read_factor]
  apply OneRead.prefix (effective_plan shares rs config.bare.privilege config.mprv)
  rw [SupervisorMemOuter.read_supervisor]
  apply OneRead.bind (value := fun word => .Ok (word, ()))
  · apply widen (checked_boundary (physical shares) rs .data address config.tor config.range config.disabled region config.matched config.readable aligned)
    intro cell member
...
theorem translate_boundary (shares : Shares) (rs : RegisterFile) (address : BitVec 64)
    ...
    (aligned : is_aligned_paddr (.Physaddr address) 8 = true) :
    OneRead (footprint shares) rs address
      (translate_and_read_value (.Virtaddr address) 8 (.Load .Data) false false false)
  ...
  have trans : RegisterPlan.Returns (footprint shares) rs
    ...
    apply widen_plan (SupervisorBare.mprv_zero_plan shares.translation rs config.bare address _ .load config.mprv)
    intro cell member
    exact List.mem_append_left _ member
  unfold translate_and_read_value
  apply OneRead.prefix trans
  apply OneRead.bind (outer_boundary shares rs address region config aligned)
  intro word; rfl
...
theorem mode_plan (shares : Shares) (rs : RegisterFile) (config : SupervisorBare.Config rs) :
  ...
  apply SupervisorBare.mode_plan rs shares.translation.status shares.translation.satp
...
theorem program_boundary (shares : Shares) (rs : RegisterFile) (address : BitVec 64)
    (region : PMA_Region) (config : Config rs address region)
    (aligned : TsoContextWord.Aligned address) :
    OneRead (footprint shares) rs address (program address) (fun word => .Ok word) := by
  have ha : is_aligned_vaddr (.Virtaddr address) 8 = true := SupervisorRead.alignment address aligned
  have page := SupervisorBareWrite.split_page_eight address aligned
  have mode := mode_plan shares rs config.bare
  have effective := effective_plan shares rs config.bare.privilege config.mprv
  have read := translate_boundary shares rs address region config (SupervisorRead.alignment address aligned)
  unfold program vmem_read_addr _root_.Sail.SailME.run PreSail.PreSailME.run
  ...
  · apply prefix_except (value := ((8, 0) : Int × Int)) (pure_plan (footprint shares) rs _)
    ...
    apply prefix_except (value := (0#64)) (pure_plan (footprint shares) rs _)
    apply OneRead.bind (value := fun word => (Except.ok word : Except Result (BitVec 64)))
    · apply OneRead.bind (one_lift read Result)
      intro word
      change (pure (Except.ok (Sail.BitVec.updateSubrange (0#64) 63 0 word)) : SailM (Except Result (BitVec 64))) = _
      rw [SupervisorRead.full_word]
    · intro word; rfl
  · intro word; rfl
