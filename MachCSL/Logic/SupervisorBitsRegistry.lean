...
/-- One source bit camera, shared by SIE/SPP/SPIE at distinct runtime names. -/
def registry : BundledGFunctors := FsCrash.registry.set slot bitFunctor

theorem registry_other (i : Nat) (different : i ≠ slot) :
    registry i = FsCrash.registry i := by
  simp [registry, BundledGFunctors.set, different]
theorem registry_old (i : Nat) (bound : i < 44) : registry i = FsCrash.registry i :=
  registry_other i (by unfold slot; omega)
theorem registry_unused (i : Nat) (bound : 45 ≤ i) : registry i = FsCrash.registry i :=
  registry_other i (by unfold slot; omega)
...
@[reducible] def bitSlot : ElemG registry BitRF := ⟨44, rfl⟩
def nativeCapacity : Capacity registry := ⟨eraCapacity.registers, { elemG := bitSlot }⟩
theorem bit_slot : nativeCapacity.bits.elemG.τ = 44 := rfl
theorem physical_registers_same : nativeCapacity.registers = machineCapacity.era.registers := rfl
