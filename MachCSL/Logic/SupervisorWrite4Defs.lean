...
def request (address : BitVec 64) (word : BitVec 32) : MemoryWriteWP.WriteRequest 4 :=
  { access_kind := .AK_explicit { variety := .AV_plain, strength := .AS_normal }
    va := none, pa := address, translation := (), size := 4, value := some word, tag := none }

abbrev Result := _root_.Sail.Result Bool (physaddr × ExceptionType)

def program (address : BitVec 64) (word : BitVec 32) : SailM Result :=
  checked_mem_write (.Physaddr address) 4 word (.Store .Data)
...
inductive Boundary (fp : RegisterFootprint.Footprint) (rs : RegisterFile)
    (req : MemoryWriteWP.WriteRequest 4) : SailM α → (MemoryWriteWP.WriteResult → SailM α) → Prop
  | event {k} : Boundary fp rs req (.impure (.writeMem 4 req) k) k
...
def OneWrite (fp : RegisterFootprint.Footprint) (rs : RegisterFile)
    (address : BitVec 64) (word : BitVec 32) (program : SailM α) (value : Bool → α) : Prop :=
