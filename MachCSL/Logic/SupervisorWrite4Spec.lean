...
structure Spec {GF : BundledGFunctors} [Platform] {hlc : HasLC} [InvGS_gen hlc GF]
  ...
  write : ∀ (shares : Shares) (rs : RegisterFile) (address : BitVec 64),
    is_aligned_paddr (.Physaddr address) 4 = true → Machine.SupervisorPmp.TorRam rs → SupervisorPhysical.RamRange address 4 →
    rs .htif_tohost_base = none →
    ∀ region : PMA_Region,
    matching_pma_region (rs .pma_regions) (.Physaddr address) 4 = some region →
