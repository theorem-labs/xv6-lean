...
theorem announcement (address : BitVec 64) :
    write_ram_ea .Write_plain (.Physaddr address) 8 = () := rfl
...
theorem program_plan (shares : Shares) (rs : RegisterFile) (address : BitVec 64)
  ...
  have hpma := SupervisorWrite.priority_store_plan rs shares.pma
    (show (.pma_regions, shares.pma) ∈ fp by simp [fp, footprint]) address region
  ...
  have hpmp : RegisterPlan.Returns fp rs
    ...
    apply widen (Logic.SupervisorPmp.check_ram_plan (shares.cfg, shares.addr) rs config.tor
      address 8 config.range.1 config.range.2.1 config.range.2.2 (.Store .Data) .store)
    intro cell member
  ...
  refine except_bind (lift_except (value := rs .mstatus)
    (RegisterPlan.Plan.read (dq := shares.status) (by simp [footprint])
      (.pure ⟨rfl, rfl⟩)) Result) ?_
  refine except_bind (lift_except (value := rs .cur_privilege)
    (RegisterPlan.Plan.read (dq := shares.privilege) (by simp [footprint])
      (.pure ⟨rfl, rfl⟩)) Result) ?_
  ...
  refine except_bind (a := ((1, 8) : Int × Int)) (pure_plan fp rs _) ?_
