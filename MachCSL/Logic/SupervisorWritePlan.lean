...
/-- Actual Data-store PMA grant, with the non-conditional assertion retained. -/
theorem pma_store_plan {fp : RegisterFootprint.Footprint} (rs : RegisterFile)
    ...
    RegisterPlan.Returns fp rs (pmaCheck (.Physaddr address) 8 (.Store .Data) .PBMT_PMA false)
...
theorem priority_store_plan {fp : RegisterFootprint.Footprint} (rs : RegisterFile)
    ...
    RegisterPlan.Returns fp rs
      (check_pma_with_pmp_priority (.Store .Data) .PBMT_PMA .Supervisor (.Physaddr address) 8 false)
      (.Ok SupervisorPhysical.alignedInfo) rs := by
...
theorem mmio_store_plan {fp : RegisterFootprint.Footprint} (rs : RegisterFile)
    ...
    RegisterPlan.Returns fp rs (within_mmio_writable (.Physaddr address) 8) false rs := by
  unfold within_mmio_writable
  simp only [show get_config_rvfi () = false from rfl, Bool.false_eq_true, ↓reduceIte]
  rw [SupervisorPhysical.clint_ram address 8 range, SupervisorPhysical.sig_ram address 8]
  refine returns_bind (pure_plan fp rs false) ?_
  refine returns_bind (pure_plan fp rs false) ?_
  refine returns_bind (SupervisorPhysical.htif_none_plan rs dq member disabled address 8) ?_
  exact pure_plan fp rs _
...
theorem write_ram_boundary (fp : RegisterFootprint.Footprint) (rs : RegisterFile)
    ...
    OneWrite fp rs address word (write_ram .Write_plain (.Physaddr address) 8 word ()) id := by
  refine ⟨_, .event, ?_, ?_⟩
  · intro result; rfl
  · rfl
...
theorem full_word (word : BitVec 64) : Sail.BitVec.extractLsb word 63 0 = word := by
...
theorem checked_boundary (shares : Shares) (rs : RegisterFile) (address word : BitVec 64)
  ...
  have hpma := priority_store_plan rs shares.pma
    (show (.pma_regions, shares.pma) ∈ fp by simp [fp, footprint]) address region matched grant aligned
  have hpmp : RegisterPlan.Returns fp rs
      (pmpCheck (.Physaddr address) 8 (.Store .Data) .Supervisor) none rs := by
      ...
      · apply prefix_except (value := ()) (pure_plan fp rs _)
        ...
        · refine OneWrite.prefix (lift_except hmmio _) ?_
          ...
          change OneWrite fp rs address word
            (((monadLift (write_ram .Write_plain (.Physaddr address) 8
...
theorem value_priv_meta_eq (address word : BitVec 64) :
  ...
  change (checked_mem_write (.Physaddr address) 8 word (.Store .Data)
    .PBMT_PMA .Supervisor () false false false >>= Pure.pure) = _
  exact EventPlan.sail_bind_pure_eq _

theorem value_priv_eq (address word : BitVec 64) :
    mem_write_value_priv (.Physaddr address) 8 word .Supervisor (.Store .Data)
