import MachCSL.Logic.TsoContextBytesReadWPSpec
...
theorem step_inv [Platform] (image : BootImage) (g : State) (gen : Nat) (cpu : CPU) (n : Nat)
    (req : MemoryReadWP.ReadRequest n) (k : MemoryReadWP.ReadResult n → SailM Unit)
    (word : BitVec (8 * n)) (ram : deviceAddress req.pa = false)
    ...
    (step : Step image (.hart gen cpu (.impure (.readMem n req) k)) g events next after forks) :
  ...
  obtain ⟨view, actual, lower, upper, reads, nextEq, stateEq, forkEq, eventEq⟩ :=
    MemoryReadWP.plain_step_inv image g gen cpu n req k ram plain live events next after forks step
...
theorem power_word_read (fixed : MachineInterp.FixedNames) (g : State) (gen : Nat)
    (era : Era.Record) (cpu : CPU) (ξ : TsoContext.CtxId) (n : Nat) (live : ThreadLive g gen)
    (a : PhysicalAddress) (dq : DFrac) (word : BitVec (8 * n)) :
    iprop(⊢ MachineInterp.powerInterp capacity fixed g -∗
      MachineInterp.generationCertificate capacity fixed gen era -∗
      running capacity era cpu ξ -∗ window capacity era ξ a n dq word -∗
      MachineInterp.powerInterp capacity fixed g ∗ running capacity era cpu ξ ∗
      window capacity era ξ a n dq word ∗ ⌜TsoContextBytes.Readback g cpu a n word⌝) := by
  ...
  have gate := TsoContextBytes.load_fact (contextCapacity capacity) (contextNames era)
    cpu ξ era.imageBytes g a n dq word
...
theorem wp_read [Platform] {hlc : HasLC} [InvGS_gen hlc GF]
    image fixed whole gen era cpu ξ n (req : MemoryReadWP.ReadRequest n)
  ...
  iintro %g %live Hp
  ihave ⟨Hp, Hrun, Hword, %reads⟩ := power_word_read capacity fixed g gen era cpu ξ n live
