import MachCSL.Logic.TsoContextWriteWPSpec
...
theorem ordinary_view (g : State) (cpu : CPU) (req : MemoryWriteWP.WriteRequest 8)
...
theorem write_views (g : State) (cpu : CPU) (req : MemoryWriteWP.WriteRequest 8)
    (word : BitVec 64) (plain : accessExclusive req.access_kind = false) :
...
theorem write_transition (g : State) (cpu : CPU) (req : MemoryWriteWP.WriteRequest 8)
    (word : BitVec 64) (plain : accessExclusive req.access_kind = false) :
    TsoContextStore.OrdinaryTransition g (MemoryWriteWP.writeState g cpu req word)
      (TsoStore.windowMap req.pa 8 word) cpu := by
  ...
  · simpa only [MemoryWriteWP.writeState, TsoStore.windowMap_decode] using
      writeBytes_overlay g.memory req.pa 8 word
...
/-- Pay the actual registered eight-byte update while retaining full heap
...
theorem bundle_store (era : Era.Record) (g : State) (cpu : CPU) (ξ : TsoContext.CtxId)
    ...
    iprop(⊢ MemoryWriteWP.writeBundle capacity.era era g -∗
      ...
      ⌜TsoContextWord.Readback (MemoryWriteWP.writeState g cpu req new) cpu req.pa new⌝) := by
  ...
  have gate := TsoContextWord.ordinary (contextCapacity capacity) (contextNames era) cpu ξ
    era.imageBytes g (MemoryWriteWP.writeState g cpu req new) req.pa old new
...
theorem wp_write [Platform] {hlc : HasLC} [InvGS_gen hlc GF]
    image fixed whole gen era cpu ξ (req : MemoryWriteWP.WriteRequest 8)
  ...
  iapply MemoryWriteWP.wp_write capacity (MemoryWriteWP.nativeContracts capacity)
    image fixed whole gen era cpu 8 req new k rr post present ram $$ Hcert Hresv
  ...
  iintro Hback
  iintro !>
  imod bundle_store capacity era g cpu ξ req old new plain $$ Hbundle Htso Hrun Hword
