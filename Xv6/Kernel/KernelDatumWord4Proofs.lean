...
theorem choose era tier ξ va dq value :
  ...
  have get := (BigSepL.bigSepL_lookup_acc (PROP := IProp GF)
    ...
    (l := List.range 4) (i := 0) (x := 0) (by rfl)).mp
  ...
  have same : (List.range 4).set 0 0 = List.range 4 := rfl
...
theorem close era tier ξ va dq value ppn (aligned : Aligned va) :
    iprop(⊢ claims capacity era tier va ppn -∗
      physicalWord capacity era ξ (physical ppn va) dq value -∗
      word capacity era tier ξ va dq value) := by
  unfold claims physicalWord TsoContextBytesReadWP.window TsoContextBytes.window word
  ...
  intro k j lookup
  have bound : j < 4 := List.mem_range.mp (List.mem_of_getElem? lookup)
  have pas := physical_offset va ppn j aligned bound
...
theorem access era tier ξ va dq value :
    iprop(word capacity era tier ξ va dq value ⊢ ∃ ppn,
      ...
      (∀ newValue, physicalWord capacity era ξ (physical ppn va) dq newValue -∗
        word capacity era tier ξ va dq newValue)) := by
  iintro Hword
  ihave ⟨%ppn,#Hhead,Hword⟩ := choose capacity era tier ξ va dq value $$ Hword
  iunfold word at Hword
  icases Hword with ⟨%aligned,Hbytes⟩
  ihave Hnormalized : iprop([∗list] j ∈ List.range 4,
      claim capacity era tier (addressAdd va j) ppn ∗
  ...
  iframe Hclaims
  isplitl [Hphysical]
  · unfold physicalWord TsoContextBytesReadWP.window TsoContextBytes.window
