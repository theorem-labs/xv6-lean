import Xv6.Kernel.KptMemory4Geometry
...
theorem mode shares rs data root (ambient : Ambient rs)
    ...
    RegisterPlan.Returns (KptAddress.footprint shares) (KptAddress.prepare rs data)
      (translationMode .Supervisor) .Sv39 (KptAddress.prepare rs data) :=
  KptMemory.mode shares rs data root ambient rooted

theorem effective shares rs data (ambient : Ambient rs) kind :
    RegisterPlan.Returns (KptAddress.footprint shares) (KptAddress.prepare rs data)
      (SupervisorMemOuter.effective (access kind)) .Supervisor (KptAddress.prepare rs data) :=
  KptMemory.effective shares rs data ambient kind

theorem read_boundary [Platform] shares rs data root (ambient : Ambient rs)
    (rooted : KptResidue.SatpRooted root data.satp) va (aligned : KernelDatumWord4.Aligned va) :
    Boundary (KptAddress.footprint shares) (KptAddress.prepare rs data)
      (KptAddress.program va (.Load .Data)) (addressProgram .load va 0#32) (readAfterAddress va) := by
  have ha : is_aligned_vaddr (.Virtaddr va) 4 = true := alignment va aligned
  have page := split_page_four va aligned
  ...
  · apply prefix_except (value := ((4, 0) : Int × Int)) (pure_plan _ _ _)
    ...
    apply prefix_except (value := (0#32)) (pure_plan _ _ _)
    ...
    | pure response =>
      ...
      | Ok pair =>
        ...
        generalize hread : mem_read (.Load .Data) pbmt pa
          (if false = true then (4 : Int) else ↑(4 : Nat)).toNat false false false = read
        have right : mem_read (.Load .Data) pbmt pa 4 false false false = read := hread
        rw [right]
        clear hread right
        induction read with
        | impure event k ih => exact congrArg (Sail.ArchSem.FreeM.impure event) (funext ih)
        | pure response =>
          cases response with
          | Ok word =>
            change pure (Except.ok (.Ok (Sail.BitVec.updateSubrange (0#32) 31 0 (word.setWidth 32)))) =
              (pure (Except.ok (.Ok word)) : SailM (Except (Result .load) (Result .load)))
            have cast : word.setWidth 32 = word := BitVec.setWidth_eq word
            have full : Sail.BitVec.updateSubrange (0#32) 31 0 (word.setWidth 32) = word :=
              (SupervisorFetchRead.full_word (0#32) (word.setWidth 32)).trans cast
            exact congrArg (fun value : BitVec 32 =>
...
theorem write_boundary [Platform] shares rs data root (ambient : Ambient rs)
    (rooted : KptResidue.SatpRooted root data.satp) va new (aligned : KernelDatumWord4.Aligned va) :
    Boundary (KptAddress.footprint shares) (KptAddress.prepare rs data)
  ...
  · apply prefix_except (value := ((4, 0) : Int × Int)) (pure_plan _ _ _)
    ...
    | pure response =>
      ...
      | Ok pair =>
        ...
        dsimp only [Bind.bind, Pure.pure, Sail.ArchSem.FreeM.bind, Function.comp_def, ExceptT.bindCont,
          _root_.Sail.assert, PreSail.assert, is_store_conditional, Bool.and, sys_misaligned_order_decreasing, writeAfterAddress]
        generalize hea : mem_write_ea pa (if false = true then (4 : Int) else ↑(4 : Nat)).toNat
          (.Store .Data) pbmt false false false = ea
        have right : mem_write_ea pa 4 (.Store .Data) pbmt false false false = ea := hea
        rw [right]
        clear hea right
        induction ea with
        | impure event k ih => exact congrArg (Sail.ArchSem.FreeM.impure event) (funext ih)
        | pure response =>
          ...
          _root_.Sail.assert, PreSail.assert, is_store_conditional, Bool.and, sys_misaligned_order_decreasing, writeAfterEA]
            generalize hwrite : mem_write_value pa
              (if false = true then (4 : Int) else ↑(4 : Nat)).toNat
              ((Sail.BitVec.extractLsb new ((8 * (if false = true then (4 : Int) else ↑(4 : Nat))) - 1).toNat 0).setWidth
                (8 * (if false = true then (4 : Int) else ↑(4 : Nat)).toNat))
              (.Store .Data) pbmt false false false = written
            have sliced : (Sail.BitVec.extractLsb new 31 0).setWidth 32 = new := by
              rw [SupervisorWrite4.full_word, BitVec.setWidth_eq]
            have right : mem_write_value pa 4 new (.Store .Data) pbmt false false false = written := by
              have equal : mem_write_value pa 4 ((Sail.BitVec.extractLsb new 31 0).setWidth 32)
...
theorem address [Platform] shares rs data root (ambient : Ambient rs)
    (rooted : KptResidue.SatpRooted root data.satp) va new kind (aligned : KernelDatumWord4.Aligned va) :
    Boundary (KptAddress.footprint shares) (KptAddress.prepare rs data)
