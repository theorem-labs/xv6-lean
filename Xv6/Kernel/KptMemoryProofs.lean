import Xv6.Kernel.KptMemoryDataProofs
...
theorem completed_tail shares rs data root (ambient : Ambient rs) kind tier ξ va ppn dq old new rr outcome
    ...
    (aligned : TsoContextWord.Aligned (KernelDatum.physical ppn va))
    ...
    iprop(⊢ MachineInterp.generationCertificate capacity.machine fixed gen era -∗
      ...
      TsoContextReadWP.wordPointsto capacity.machine era ξ (KernelDatum.physical ppn va) dq old -∗
      (∀ value, TsoContextReadWP.wordPointsto capacity.machine era ξ (KernelDatum.physical ppn va) dq value -∗
...
theorem wp_address shares rs (ambient : Ambient rs) kind tier ξ va dq old new rr
  ...
  ihave ⟨%ppn,Hclaims,Hword,Hclose⟩ := (KernelDatumWord.nativeSpec capacity).access era tier ξ va dq old $$ Hword
  ihave Hclaim := (KernelDatumWord.nativeSpec capacity).head era tier va ppn $$ Hclaims
  iunfold KernelDatum.claim at Hclaim
  icases Hclaim with ⟨#Hmap,%facts⟩
  obtain ⟨positive,ram,_pin⟩ := facts
  have alignedPhysical := KernelDatumWord.physical_aligned va ppn aligned
