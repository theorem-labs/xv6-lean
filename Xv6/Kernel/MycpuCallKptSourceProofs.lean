...
theorem wp_call (jal : KptJalSource.Spec capacity) (code : KptJal.ResourceSpec capacity)
  ...
  iapply jal.cycle image fixed whole gen era cpu tier ξ original available pc imm tick jalFrame post
    (targetEven pc imm target) $$ [Hopened Hcode Htext Hpma Hextra] [Hfinish]
  · unfold KptJalSource.input
    dsimp only [jalFrame]
    iframe
  · iunfold KptJalSource.finish
    iintro Hreturned %bodyTick
    iunfold KptJalSource.restored at Hreturned
    ...
    · iunfold MycpuSconf.finish
      ...
      isimp only [returnPC cpu pc original aligned] at Hsource
      iunfold finish at Hfinish
      iapply Hfinish $$ %after %(result cpu pc original after good)
        [Hsource Hcode Htext Hpma Hextra] %nextTick
      iunfold restored
      iframe

theorem actual (jal : KptJalSource.Spec capacity) (code : KptJal.ResourceSpec capacity) : Spec capacity :=
