...
theorem load_factor [Platform] (slot : Slot) : body .load slot = (do
    ...
    KptMemory.program .load (sp + MycpuMemory.offset slot) 0#64 >>= loadTail slot) := by
  rw [load_body]
  unfold execute_LOAD
  rw [MycpuMemory.immediate_eq]
  change (pure () >>= fun _ => vmem_read (.Regidx 2#5) (MycpuMemory.offset slot) 8 (.Load .Data) false false false >>= _) = _
  ...
  conv =>
    rhs
    change Sail.ArchSem.FreeM.bind (Sail.ArchSem.FreeM.bind (transform_effective_address (.Virtaddr (sp + MycpuMemory.offset slot)) (.Load .Data)) (fun va => (vmem_read_addr va 8 (.Load .Data) false false false : SailM (KptMemory.Result .load)))) (loadTail slot)
    rw [free_assoc]
  apply congrArg (fun k : virtaddr → SailM ExecutionResult => Sail.ArchSem.FreeM.bind (transform_effective_address (.Virtaddr (sp + MycpuMemory.offset slot)) (.Load .Data)) k)
  funext va
  ...
  | pure response =>
    ...
    | Ok word =>
      ...
      change (wX_bits (MycpuMemory.dataIndex slot) (extend_value false word) >>= fun _ => pure (.Retire_Success ())) = MycpuMemory.loadTail slot word
      rw [extend]
      rfl
    | Err error => rfl
  | impure e k ih =>
...
theorem store_factor [Platform] (slot : Slot) : body .store slot = (do
    ...
    let sp ← rX_bits (.Regidx 2#5)
    KptMemory.program .store (sp + MycpuMemory.offset slot) new >>= storeTail) := by
  rw [store_body]
  unfold execute_STORE
  rw [MycpuMemory.immediate_eq]
  change (pure () >>= fun _ => rX_bits (MycpuMemory.dataIndex slot) >>= fun new => pure (Sail.BitVec.extractLsb new 63 0) >>= fun new => vmem_write (.Regidx 2#5) (MycpuMemory.offset slot) 8 (BitVec.setWidth 64 new) (.Store .Data) false false false >>= storeTail) = _
  simp only [BootPmp.sail_pure_bind, SupervisorWrite.full_word, BitVec.setWidth_eq]
  apply congrArg (fun k : BitVec 64 → SailM ExecutionResult => rX_bits (MycpuMemory.dataIndex slot) >>= k)
  funext new
  rw [virtual_write_factor]
  exact BootPmp.sail_bind_assoc (rX_bits (.Regidx 2#5)) (fun sp => KptMemory.program .store (sp + MycpuMemory.offset slot) new) storeTail

theorem pureSpec [Platform] : PureSpec :=
  ⟨store_body, load_body, store_factor, load_factor, store_false, store_error,
    load_error, footprint_unique, footprint_counts, footprint_members, ambient,
    map_other, after_sp, after_address⟩
