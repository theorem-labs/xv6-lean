...
theorem wp_body instruction shares control values (config : Config instruction control)
  ...
  ihave ⟨Hcells,Hbits,Hz,Htr⟩ :=
    (MycpuRegimeShell.partition capacity era cpu (.kpt N root) control values shares).mp $$ Hpacket
  ...
  ihave Hpacket := (MycpuRegimeShell.partition capacity era cpu (.kpt N root)
    (afterControl instruction control cpu values) (afterValues instruction control cpu values) shares).mpr
    $$ [Hcells Hbits Hz Htr]
  · rw [status, zero]
    iframe
  iapply Hfinish
