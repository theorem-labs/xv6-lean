...
theorem wp_function (functionSpec : MycpuKpt.Spec capacity) (phaseSpec : MycpuKpt.PureSpec)
    image fixed whole gen era cpu ξ original available tick (frame : IProp GF) post
    (enough : 2 ≤ available) :
  ...
  ihave Hfull := (KernelTextImage.nativeSpec capacity.translation).mono era .identity .full (by trivial) $$ Htext
  ihave ⟨_,Hcode⟩ := (KernelTextImage.nativeSpec capacity.translation).mycpu era .full capacity rfl $$ Hfull
  ihave Hentry : MycpuKptEntry.input capacity fixed gen era cpu ξ original available $$ [Hsource Hcode]
  · unfold MycpuKptEntry.input
    iframe Hsource Hpma Hcode
  ihave ⟨%root,%initial,%old,%rr,%config,%boundary,Hresources⟩ :=
    (MycpuKptEntry.nativeSpec capacity).open_entry fixed gen era cpu ξ original available enough $$ Hentry
  ihave ⟨#Hcert,Hresources⟩ := (MycpuKptEntry.nativeSpec capacity).certificate fixed gen era cpu ξ
    (MycpuKptEntry.sp original) available root initial original old rr $$ Hresources
  iunfold MycpuKptEntry.resources at Hresources
  icases Hresources with ⟨Hpacket,Hcode,Hrun,Hpair,Hresv,HentryFrame⟩
  let extra : IProp GF := iprop(MycpuKptEntry.frame capacity fixed gen era cpu ξ
    (MycpuKptEntry.sp original) available ∗ frame)
  have entrySP : MycpuKpt.entrySP cpu original = MycpuKptEntry.sp original := rfl
  iapply functionSpec.function MycpuRegimeShell.sourceShares initial original config boundary.pc_eq
    old .full ξ rr tick image fixed whole gen era cpu KptGhost.kptN root extra post
  ...
  · iunfold MycpuKpt.finish
    ...
    have sourceResult := result initial original cpu after values good.1
    have restoredSP := stack initial original cpu after values good.1
    have afterBoundary := phaseSpec.boundary initial original cpu after values good.1 boundary
    ihave Hentry : MycpuKptEntry.resources capacity fixed gen era cpu ξ (MycpuKptEntry.sp original)
        available root after values (MycpuKpt.savedWords cpu original) none $$
        [Hpacket Hcode Hrun Hpair Hresv HentryFrame]
    · unfold MycpuKptEntry.resources
      isimp only [entrySP] at Hpair
      iframe
    ihave Hrestored := (MycpuKptEntry.nativeSpec capacity).close_entry fixed gen era cpu ξ
      (MycpuKptEntry.sp original) available root after values (MycpuKpt.savedWords cpu original) none
      (returnPC cpu original) enough restoredSP afterBoundary $$ Hentry
    iunfold MycpuKptEntry.restored at Hrestored
