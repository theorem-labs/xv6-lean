...
def offset (site : Site) : Int := if site.val = 0 then 0x10 else 0x18
...
def immediate (site : Site) : BitVec 21 := if site.val = 0 then 3370#21 else 3362#21
def encoding (site : Site) : BitVec 32 := if site.val = 0 then 0x52b000ef#32 else 0x523000ef#32
