...
def check_vext_config (_ : Unit) : Bool :=
  ...
  let valid : Bool :=
    if (((false : Bool) && (not (hartSupports Ext_Zve32f))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
  ...
  let valid : Bool :=
    if (((false : Bool) && ((not (hartSupports Ext_Zve32f)) || (not (true : Bool)))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
...
/-- Type quantifiers: k_ex744451_ : Bool -/
...
/-- Type quantifiers: k_ex744606_ : Bool -/
...
def check_misc_extension_dependencies (_ : Unit) : Bool :=
  ...
  let valid : Bool :=
    if (((false : Bool) && ((not (true : Bool)) || (not (false : Bool)))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
